...
class BankTransactionCategorizer:
    def __init__(self, model_path='jonngan/distilbert-transaction-classifier-v4', rules_path='scripts/category_rules.json'):
    ...
    def predict(self, df):
        ...
        df['Category_Confidence'] = 1.0  # Rule-based predictions get full confidence
        df['Sub_Category_Confidence'] = 1.0
        ...
        if not unclassified_df.empty:
            ...
            all_cat_confidences, all_sub_confidences = [], []
            ...
            with torch.no_grad():
                for batch in predict_dataloader:
                    ...
                    cat_confidences = torch.max(cat_probs, dim=1)[0].cpu().numpy()
                    sub_confidences = torch.max(sub_probs, dim=1)[0].cpu().numpy()

                    all_cat_preds.extend(cat_preds)
                    all_sub_preds.extend(sub_preds)
                    all_cat_confidences.extend(cat_confidences)
                    all_sub_confidences.extend(sub_confidences)
            ...
            df.loc[unclassified_mask, 'Category_Confidence'] = all_cat_confidences
            df.loc[unclassified_mask, 'Sub_Category_Confidence'] = all_sub_confidences

        df.fillna({'Category': 'Unclassified_Miscellaneous', 'Sub_Category': 'Unknown', 'Category_Confidence': 0.0, 'Sub_Category_Confidence': 0.0}, inplace=True)
        return df 